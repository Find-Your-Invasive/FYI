...
#Questions
feature= input ("Type 1 for Invasive Tracker, Type 2 for Add Your Invasive, Type 3 for Create Your Invasive: ")
if feature=="1":
...
if feature=="1":
    ...
    if model=="2" or model=="3" or model=="4" or model=="5":
        ...
        if stringer=="1":
            ...
            if model == "2":
                futureNeural(finalresult, codeval)
            elif model == "3":
                futureLinear(finalresult, codeval)
            elif model == "4":
                futureLogit(finalresult, codeval)
            elif model == "5":
                print ("Hi")

                print("Hi2")
                b = futureLinear(finalresult, codeval)
                c = futureLogit(finalresult, codeval)
                d = futureRandomForests(finalresult, codeval)
                a = futureNeural(finalresult, codeval)
                print("Hi3")
                ...
                for x in range(len(stats)):
                    if stats[x] > mean + 2 * sd or stats[x] < mean - 2 * sd:
                        print("We will delete this entry: " + stats[x])
                        stats.remove(stats[x])
